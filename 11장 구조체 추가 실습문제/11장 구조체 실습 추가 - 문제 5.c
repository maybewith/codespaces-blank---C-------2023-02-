...
int main() {
    ...
    for(int i=0;i<n;i++){
        scanf("%s %s %d %d",s[i].name,s[i].year,&s[i].pen,&s[i].face);
    ...
    for(int i=0;i<n-1;i++){
        for(int j=i+1;j<n;j++){
            ...
            // 만약 합이 같다면
            if(s[i].final==s[j].final&&s[i].pen<s[j].pen){
                swap((s+i),(s+j));
            }
            
            // 필기시험 점수와 면접시험 점수도 모두 같은 경우
            if(s[i].final==s[j].final&&s[i].pen==s[j].pen&&s[i].face==s[j].face&&strcmp(s[i].year,s[j].year)<0){
                swap((s+i),(s+j));
            }
        }
    }
